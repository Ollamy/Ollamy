...
const LessonList = (props: LessonListProps) => {
  ...
  useEffect(() => {
    if (selectedLessonId) {
      if (sectionId) navigate(`/course/${courseId}/${sectionId}/${selectedLessonId}`);
    }
  }, [courseId, navigate, sectionId, selectedLessonId]);
...
const LessonBox = styled.div<{ is_selected: boolean }>`
  ...
  box-shadow: 2px 2px 8px 0 rgba(189, 189, 189, 0.25);
