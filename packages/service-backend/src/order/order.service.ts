...
function midpoint(a: string, b: string | null | undefined, digits: string): string {
    ...
    if (digitB - digitA > 1) {
        ...
        const firstDigit = a ? digits.indexOf(a[0]) : 0;
        if (b && b.length > 1) {
            return b.slice(0, 1);
        } else {
            return b ? digits[firstDigit] + midpoint(a.slice(1), null, digits) : digits[firstDigit];
        }
    }
}
...
export function InitOrderKey(items: string[]) {
    ...
    for (let i = 0; i < items.length - 1; i++) {
        keys.push(generateKeyBetween(keys[i], null));
    }
